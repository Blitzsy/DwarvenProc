    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException
    {
        if (sender instanceof EntityPlayer)
        {
            ...
            if (player.getEntityData().hasKey("procStreak"))
                player.addChatComponentMessage(new TextComponentTranslation(Messages.CHAT_MESSAGE_PROC_COUNT, TextFormatting.WHITE + String.valueOf(player.getEntityData().getInteger("procStreak"))).setStyle(new Style().setColor(TextFormatting.GREEN)));
            else
