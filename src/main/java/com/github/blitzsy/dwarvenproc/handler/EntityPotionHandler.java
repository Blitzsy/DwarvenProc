        ...
        if (event.getEntityLiving() instanceof EntityPlayer && !event.getEntityLiving().isPotionActive(Potions.dwarvenProcEffect))
        {
            ...
            if (player.getEntityData().hasKey("procKills"))
            {
                ...
                if (procKills > 0)
                {
                    ...
                    {
                        player.sendMessage(new TextComponentTranslation(Messages.CHAT_MESSAGE_PROC_COUNT, TextFormatting.WHITE + String.valueOf(procKills)).setStyle(new Style().setColor(TextFormatting.GREEN)));
                    }
