        ...
        if (event.getEntityLiving() instanceof EntityPlayer && !event.getEntityLiving().isPotionActive(Potions.dwarvenProcEffect))
        {
            ...
            if (player.getEntityData().hasKey("procKills"))
            {
                ...
                if (procKills > 0)
                {
                    ...
                    {
                        player.addChatComponentMessage(new TextComponentTranslation(Messages.CHAT_MESSAGE_PROC_COUNT, TextFormatting.WHITE + String.valueOf(procKills)).setChatStyle(new Style().setColor(TextFormatting.GREEN)));
                    }
